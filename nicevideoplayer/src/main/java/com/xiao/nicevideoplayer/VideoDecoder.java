...
public class VideoDecoder {
    ...
    public boolean decodeVideo(String url, String newFilePaht, long clipPoint, long clipDuration) {
        ...
        try {
            ...
            mediaMuxer = new MediaMuxer(newFilePaht, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);

            // 修正合成器视频方向
            MediaMetadataRetriever metadataRetriever = new MediaMetadataRetriever();
            metadataRetriever.setDataSource(url);
            int angle = Integer.parseInt(metadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_ROTATION));
            mediaMuxer.setOrientationHint(angle);
        ...
        for (int i = 0; i < mediaExtractor.getTrackCount(); i++) {
            try {
                ...
                if (mime.startsWith("video/")) {
                    ...
                    LogUtil.i("videoDuration-------"+videoDuration);
                    //检测剪辑点和剪辑时长是否正确
