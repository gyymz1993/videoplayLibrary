...
import com.andview.listener.OnItemLongClickListener;
import com.andview.listener.OnItemTouchListener;
...
public abstract class AdapterForRecyclerView<T> extends RecyclerView.Adapter<ViewHolderForRecyclerView> {
    ...
    public int getItemViewType(int position) {
        if (mDefaultLayoutId == 0) {
            throw new RuntimeException("请在 " + this.getClass().getSimpleName() + " 中重写 getItemViewType 方法返回布局资源 id，或者使用 " + this.getClass().getSimpleName() + " 三个参数的构造方法 AdapterForRecyclerView(Context context, int defaultLayoutId, List<T> data)");
        }
    ...
    public ViewHolderForRecyclerView onCreateViewHolder(ViewGroup parent, int viewType) {
        ViewHolderForRecyclerView holder = new ViewHolderForRecyclerView(mContext, View.inflate(mContext, viewType, null));
...
//    @Override
//    public void onBindViewHolder(ViewHolderForRecyclerView holder, int position, List<Object> payloads) {
//        convert(holder, mData.get(position), position);
//    }
    ...
    public abstract void convert(ViewHolderForRecyclerView helper, T item, int position);
